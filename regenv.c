...
#define SHExpandEnvironmentStrings      ExpandEnvironmentStrings
...
BOOL SetUserEnvironmentVariable(void **pEnv, LPTSTR lpVariable, LPTSTR lpValue, BOOL bOverwrite)
{
    ...
    if (!(RtlInitUnicodeStringEx(&Name, lpVariable) >= STATUS_SUCCESS))
    {
        return FALSE;
    }
    cch = 1024;
    ...
    if (Value.Buffer) {
        ...
        if ((Status >= STATUS_SUCCESS) && !bOverwrite) {
            return(TRUE);
        }
    }
    if (lpValue && *lpValue) {
...
BOOL BuildEnvironmentPath(void **pEnv, LPTSTR lpPathVariable, LPTSTR lpPathValue)
{
    ...
    if (!(RtlInitUnicodeStringEx(&Name, lpPathVariable) >= STATUS_SUCCESS))
    {
        return FALSE;
    }
    cb = 1024;
    ...
    )
{
    ...
    if (!(RtlInitUnicodeStringEx(&Name, lpAutoexecPath) >= STATUS_SUCCESS))
    {
        return FALSE;
    }
    cch = 1024;
